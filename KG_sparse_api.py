...
class KnowledgeGraphSparse(object):
    ...
    def get_triples_along_relation_path(self, head_set, tail_set, rel_path, next_hop_num):
        ...
        deduced_nodes = self.deduce_node_leaves_by_path(head_set, rel_path) 
        if len(deduced_nodes) == 1 and head_set in deduced_nodes: 
            return []
        else:
            forw_one_tris = self._fetch_forward_triple(deduced_nodes)
            forw_one_nodes = np.unique(forw_one_tris[:, 2]) 

        if next_hop_num == 1:
            if np.intersect1d(forw_one_nodes, tail_set).size > 0:
                path_x_t = self.path_join(forw_one_tris, tgt_node, only_keep_rel=True)  
                return path_x_t
            else:
                return []
            ...
            if np.intersect1d(forw_one_nodes, back_one_nodes).size > 0:
                path_x_c_t = self.path_join(forw_one_tris, back_one_tris, only_keep_rel=True)
                return path_x_c_t
            else:
                return []
        else:
    ...
    def search_paths(self, src_set, tgt_set, hop):
        ...
        forward_triple_one = self._fetch_forward_triple(src_set) # triple id的数组
        forward_node_one = np.unique(forward_triple_one[:, 2])
        if hop == 1:
            local_path = []
        ...
        for i in range(max_hop):
            ...
            for ins_abs in last_abs_id: 
                ...
                if task_name == "kgc":
                    ...
                    const_rels = self.get_const_rels(triples, cpes, exclude_rels) # 根据triples和cpes的交集得到constraint relations 

                const_rels_str = [id2rel[rel] for rel in const_rels]
                all_const_rels.update(const_rels_str)
                candidate_rels = set(triples[:, 1].tolist()) - exclude_rels - const_rels # 除了constraint relations之外的其他relation
                ...
                if not_filter_rels:
                    ...
                    filtered_rels_str = self.get_filtered_rels(question, candidate_rels_str, tokenizer, model, topk, filter_score)
                ...
                for rel_str in filtered_rels_str:
                    ...
                    triples_per_hop[i].append((pa_id, rel_str, placeholder_id)) # 将同一个head entity和relation相连的tail entity当成一个abstract node，存储到triples_per_hop中
                    cur_abs_id.append((np.array(list(tails)), placeholder_id)) # 将abstract node和原始的entity的关系存到cur_abs_id中，可以用来检索下一hop的节点
