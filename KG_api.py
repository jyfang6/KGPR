...
from KG_sparse_api import KnowledgeGraphSparse
import pickle
...
class KnowledgeGraph(object):
    ...
    def deduce_subgraph_by_all_paths(self, kg, srcs, paths):
        ...
        :return: list of nodes and list of triples in deduced subgraph
        """
        assert len(srcs) == len(paths)  
        total_nodes, total_triples = set(), set()
        for te, ps in zip(srcs, paths): 
    ...
    def get_paths_from_tpe_to_cpes(self, source, cpes, rels_path, task_name):
        ...
        if len(before_x_common_rel_path) > 0: 
            paths_y_t = self.sparse_kg.get_triples_along_relation_path(src_id, cpes_id, before_x_common_rel_path, next_hop_num=1)
        ...
        paths_x_t = self.sparse_kg.get_triples_along_relation_path(src_id, cpes_id, after_x_common_rel_path, next_hop_num=1)
        ...
        if task_name == "cwq":
            paths_x_c_t = self.sparse_kg.get_triples_along_relation_path(src_id, cpes_id, after_x_common_rel_path, next_hop_num=2)
